#!/usr/bin/env python3
"""
Zoo-based adversarial training for Safety Gym (Buffered protagonist).

Same as train_zoo.py but protagonist uses BufferedAgent with replay buffer
and importance-weighted corrections. Adversary remains standard PPOAgent.
"""
import argparse
import json
...
def train_zoo_buffered(
    ...
    buf_cfg = BufferedConfig(
        ...
        buffer_size=buffer_size, is_ratio_clip=is_ratio_clip,
    )
    ppo_cfg = PPOConfig(obs_dim=obs_dim, act_dim=act_dim, hidden=hidden,
                        lr_actor=lr_actor, lr_critic=lr_critic)

    protagonist = BufferedAgent(buf_cfg)
    ...
    while total_steps < timesteps:
        ...
        # Store in buffer and update protagonist
        protagonist.store(prot_buf)
        prot_metrics = protagonist.update()
        ...
        if update_step % log_interval == 0:
            metrics = {
                ...
                "buffer_size": len(protagonist.buffer),
            }

            if "is_ratio" in prot_metrics:
                metrics["is_ratio"] = prot_metrics["is_ratio"]
...
def main():
    parser = argparse.ArgumentParser(description="Safety Gym zoo training (Buffered)")
